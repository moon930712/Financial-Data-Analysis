...
# 1. 2025-04-01 기준 종목 선정 (수정된 가중치 적용)
def get_top_stocks_on_date(target_date):
    ...
    ranked AS (
        SELECT 
            ...
            ROUND(
                (
                    (industry_rel_pbr_score * 0.3) + 
                    (roe_rank_score * 0.2) + 
                    (senti_rank_score * 0.2 + vol_rank_score * 0.3)
                )::numeric
            , 2) AS total_score,
...
def run_backtest():
    ...
    print(f"[{base_date}] 기준 상위 종목 추출 중...")
    ...
    summary.to_csv('backtest_result_20250401.csv', index=False, encoding='utf-8-sig')
    industry_avg.to_csv('industry_avg_result_20250401.csv', encoding='utf-8-sig')
    
    return summary, total_avg
