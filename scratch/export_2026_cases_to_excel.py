...
def main():
    ...
    # New Strict Extrema Conditions
    # Point A / C (Neg Max): Phase 2 -> 1, prev_z <= -1.0, D-2 hist > D-1 hist
    df['is_neg_max'] = (
        (df['prev_phase'] == 2) & 
        (df['phase'] == 1) & 
        (df['prev_z'] <= -1.0) &
        (df['prev2_hist'] > df['prev_hist'])
    )
    
    # Point B (Pos Max): Phase 3 -> 4, prev_z >= 1.0, D-2 hist < D-1 hist
    df['is_pos_max'] = (
        (df['prev_phase'] == 3) & 
        (df['phase'] == 4) & 
        (df['prev_z'] >= 1.0) &
    ...
    # Filter 2026-01-01 onwards based on Point C
    valid_seq = valid_seq[valid_seq['next_next_date'] >= pd.to_datetime('2026-01-01').date()]
    
    point_a = valid_seq[['stock_code', 'stock_name', 'theme_name', 'date', 'close', 'next_date', 'next_next_date']].rename(
    ...
    # Drop rows without D+5 data
    target_df = target_df.dropna(subset=['d5_ret'])
    
    # Row-level average and median
    ...
    raw_data = target_df[['theme_name', 'stock_name', 'date_A', 'date_B', 'date_C', 'diff_AC_pct', 
                          'd1_ret', 'd2_ret', 'd3_ret', 'd4_ret', 'd5_ret', 'ret_mean', 'ret_median']].copy()
                          
    raw_data.columns = [
        '테마명', '종목명', 'Point A 일자', 'Point B 일자', 'Point C 일자(매수일)', 
        'Point A 대비 하락률(%)', 'D+1 수익률(%)', 'D+2 수익률(%)', 'D+3 수익률(%)', 
    ...
    # Use full absolute path to ensure we know exactly where it goes, or relative to cwd.
    out_path = os.path.abspath('scratch/6등급_회복기_수익률분석_2026.xlsx')
