...
def main():
    ...
    print("Extracting DWM OHLCV data for Net Buying Analysis...")
    query_dwm = """
    SELECT stock_code, d_date, d_volume, d_open, d_high, d_low, d_close, w_open, w_high, w_low, w_close 
    ...
    # Calculate Buying Pressure (순매수 강도) from OHLC
    # Daily Buy Pressure = (d_close - d_low) / (d_high - d_low)
    target_df['d_buy_pressure'] = np.where(target_df['d_high'] > target_df['d_low'], 
                                           (target_df['d_close'] - target_df['d_low']) / (target_df['d_high'] - target_df['d_low']) * 100, 
                                           50) # If high == low (Limit up/down), neutral
                                           
    # Weekly Buy Pressure = (w_close - w_low) / (w_high - w_low)
    target_df['w_buy_pressure'] = np.where(target_df['w_high'] > target_df['w_low'], 
                                           (target_df['w_close'] - target_df['w_low']) / (target_df['w_high'] - target_df['w_low']) * 100, 
                                           50)
    ...
    print("\n========================================================")
    print("🔥 [6등급 + 황금 망치형] 수익/손실 그룹 '순매수 강도(Buying Pressure)' 비교")
    print("========================================================\n")
    
    def print_buy_pressure_stats(group_df, name):
        count = len(group_df)
        if count == 0: return
        
        avg_d_buy = group_df['d_buy_pressure'].mean()
        avg_w_buy = group_df['w_buy_pressure'].mean()
        
        # 80% 이상의 극단적 순매수 강도 비율
        extreme_d_buy = (group_df['d_buy_pressure'] >= 80).mean() * 100
        extreme_w_buy = (group_df['w_buy_pressure'] >= 80).mean() * 100
        
        print(f"🟢 [{name} 그룹] - 총 {count}건")
        print(f"  [1] 주간(Weekly) 순매수 강도 (해당 주간 전체 거래량 중 순매수세가 차지하는 비중)")
        print(f"      - 평균 주간 순매수 강도: {avg_w_buy:.1f}%")
        print(f"      - 주간 순매수 강도가 80% 이상 압도적인 종목 비율: {extreme_w_buy:.1f}%")
        print(f"  [2] 일간(Daily) 순매수 강도 (Point C 당일 거래량 중 순매수세가 차지하는 비중)")
        print(f"      - 평균 당일 순매수 강도: {avg_d_buy:.1f}%")
        print(f"      - 당일 순매수 강도가 80% 이상 압도적인 종목 비율: {extreme_d_buy:.1f}%")
        print("\n")

    print_buy_pressure_stats(win_group, "수익(Win)")
    print_buy_pressure_stats(loss_group, "손실(Loss)")
