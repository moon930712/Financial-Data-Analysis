...
def create_refined_document():
    ...
    p_plan1.add_run('가장 압도적인 수익을 내는 "6 - 1 패턴" 타겟팅').bold = True
    p_plan1.add_run(': 6등급에서 1등급으로 수직 상승하는 테마(섹터)들의 구조적 특징을 추출합니다.')
    
    p_plan2 = doc.add_paragraph(style='List Bullet')
    p_plan2.add_run('섹터 차트 및 보조지표 정밀 분석').bold = True
    p_plan2.add_run(': 해당 6-1 패턴을 그리는 섹터들이 수직 상승을 시작하기 직전에 어떤 "차트적 시그널"과 "보조지표 특이점"을 보였는지 집중 분석하여, 6-1 직행을 미리 예측할 수 있는 궁극적인 선행 지표를 발굴할 예정입니다.')

    out_path = os.path.abspath('theme_analysis_insight_final_v4.docx')
    doc.save(out_path)
