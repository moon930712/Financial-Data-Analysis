...
def create_refined_document():
    ...
    # 5. 투자전략
    doc.add_heading('4. 결론 및 투자 전략: 통계가 증명하는 최적의 매수 타점', level=1)
    p = doc.add_paragraph()
    p.add_run('모든 데이터 분석 결과를 종합해 내린 결론입니다. 가장 승률이 높고 자금 회전율을 극대화할 수 있는 완벽한 매수 타이밍은 ')
    p.add_run('"6등급에 철저히 소외되어 있던 테마가, 어느 날 5등급이나 4등급 혹은 그 이상으로 강하게 고개를 들며 바닥 탈출(Point A)을 확정 지었을 때"').bold = True
    p.add_run('입니다.\n')
    
    p = doc.add_paragraph()
    p.add_run('앞선 데이터가 증명하듯, 성공적으로 1등급까지 도달하는 대장 테마들은 바닥을 탈출할 때 찔끔 오르지 않고 단숨에 1~2등급으로 수직 상승하는 뚜렷한 특징(52.1%)을 보입니다. 따라서 매일 6등급 좀비 테마들을 관찰하시다가, ')
    p.add_run('어느 날 갑자기 의미 있는 거래량을 동반하며 등급이 확! 쳐드는 바로 그 첫날이나 이튿날').bold = True
    p.add_run('에 올라타는 확인 매수 전략이 실제 통계상 "수익 창출"과 "빠른 자금 회전율"을 동시에 극대화하는 방법입니다!')

    out_path = os.path.abspath('theme_analysis_insight_final_v2.docx')
    doc.save(out_path)
