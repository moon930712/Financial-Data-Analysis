-- 테마별 MACD 히스토그램 고점 반락(Peaking) 신호 발생 종목 상세 리스트
-- 작성일: 2026-04-16
WITH histo_base AS (
...
signals AS (
    ...
    WHERE 1=1
      ...
      AND s.histogram > 0 -- 양봉 구간
      AND s.lag2_histogram < s.lag_histogram -- 상승 중
      AND s.lag_histogram > s.histogram -- 하락 반전
      AND (s.lag_histogram - s.hist_avg_20) / NULLIF(s.hist_std_20, 0) >= 1.0 -- 과열 수준 (Z-Score +1.0 이상)
),
stock_info AS (
    -- 4. 시가총액 정보 결합
...
-- 7. 최종 출력: 테마별 고점 반락 종목 상세 (Grafana 연동용 industry_high 변수 적용)
SELECT 
...
WHERE 1=1
  AND tr.theme_name IN ($industry_high)
ORDER BY 
