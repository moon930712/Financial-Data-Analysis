-- 테마별 MACD 히스토그램 고점 반락(Peaking) 신호 분석 (양봉 V자 반전)
-- 작성일: 2026-04-16
WITH histo_base AS (
...
signals AS (
    ...
    WHERE 1=1
      ...
      AND s.lag_histogram > s.histogram -- 하락 반전 (V자 산 모양)
      AND (s.lag_histogram - s.hist_avg_20) / NULLIF(s.hist_std_20, 0) >= 1.0 -- 과매수 수준 (Z-Score +1.0 이상)
),
stock_info AS (
    -- 4. 시가총액 정보 결합
...
-- 7. 최종 출력: 업종명 리스트 (Distinct 결과 도출용 요약 뷰)
SELECT 
