-- WICS 중분류(wics_name2) 기준 국면 그룹핑(1+2, 3+4) 순환매 분석
...
ratio_calculation AS (
    SELECT 
        *,
        -- 회복기(1)에 1.5배 가중치를 부여한 가중 점수 (순위 산정용)
        ROUND(((phase1_cnt::numeric * 1.5 + phase2_cnt) / total_cnt) * 100, 2) as g1_weighted_score,
        ROUND(((phase1_cnt + phase2_cnt)::numeric / total_cnt) * -100, 2) as g1_ratio,
        ROUND((g2_cnt::numeric / total_cnt) * 100, 2) as g2_ratio
    FROM sector_aggregation
)
SELECT 
    to_char(date, 'YYYY-MM-DD') as "날짜",
    ROW_NUMBER() OVER(ORDER BY g1_weighted_score DESC, total_cnt DESC) as "순위",
    sector_name as "업종명(WICS중분류)",
    total_cnt as "업종전체종목수",
    g1_weighted_score as "회복가중점수",
    g1_ratio as "0선아래(회복+하락)비율(%)",
    g2_ratio as "0선위(상승+둔화)비율(%)"
FROM ratio_calculation
ORDER BY g1_weighted_score DESC, total_cnt DESC, sector_name ASC;
