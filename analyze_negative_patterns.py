...
def run_analysis():
    ...
    for idx, row in df_ranks.iterrows():
        ...
        try:
            ...
            min_return_for_theme = min(valid_returns)
                    
            if min_return_for_theme <= -3.0:
                count_success += 1
                ...
                results.append({
                    ...
                    'D-1 Rank': ranks[1],
                    'D-day Rank': ranks[2],
                    'Min 5-Day Return (%)': round(min_return_for_theme, 2),
    ...
    print(f"Events with <= -3% min return within 5 days: {count_success}")
    ...
    if results:
        ...
        # Merge avg min return per pattern
        pattern_summary = df_res.groupby('Pattern')['Min 5-Day Return (%)'].mean().reset_index()
        pattern_counts = pattern_counts.merge(pattern_summary, on='Pattern')
        
        print("\n=== Top 20 Most Frequent 5-Day Patterns Before a <= -3% Drop ===")
        print(pattern_counts.head(20).to_string(index=False))
        
        excel_path = 'negative_pattern_analysis.xlsx'
